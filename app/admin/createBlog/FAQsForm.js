...
const FAQsForm = ({ faqs = [], setFaqs }) => {
  ...
  const handleFaqChange = (faqIndex, field, value) => {
    setFaqs((prevFaqs) => {
      const updatedFaqs = Array.isArray(prevFaqs) ? [...prevFaqs] : [];
      if (!updatedFaqs[faqIndex]) return updatedFaqs; // Prevent error if index is invalid
      updatedFaqs[faqIndex] = { ...updatedFaqs[faqIndex], [field]: value };
      return updatedFaqs;
    });
  };
  ...
  const addFaq = () => {
    setFaqs((prevFaqs) => {
      const updatedFaqs = Array.isArray(prevFaqs) ? [...prevFaqs] : [];
      return [...updatedFaqs, { question: "", answer: "" }];
    });
  };
